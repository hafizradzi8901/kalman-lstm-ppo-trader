"""
PPO Training Pipeline -- Baseline + Ablation Studies

Trains a PPO agent on TradingEnv with SB3. Supports switching between
different model backbones (baseline Kalman-LSTM vs FNO-enhanced etc.)
for ablation comparison.

Fixes over v1:
  - Evaluation uses VecNormalize (was raw obs -> policy collapse)
  - Random episode start points to prevent memorizing one trajectory
  - Multi-ticker training for generalization
  - Higher entropy coefficient to prevent policy collapse
  - Proper Sharpe ratio calculation in eval
  - 5y default data (was 2y)
...
# ============================================================
# Feature extractors for SB3
# ============================================================
class KalmanLSTMExtractor(BaseFeaturesExtractor):
    """Baseline: Kalman-LSTM cell as feature extractor."""

    def __init__(self, observation_space: spaces.Box, hidden_size: int = 64):
        super().__init__(observation_space, features_dim=hidden_size)
        obs_size = observation_space.shape[0]
        self.cell = KalmanLSTMCell(obs_size, hidden_size)
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        h, c, p = self.cell(observations, h, c, p)
        return h


class FNOKalmanLSTMExtractor(BaseFeaturesExtractor):
    """FNO-enhanced: Kalman-LSTM -> FNO filtering as feature extractor.

    Since SB3 feeds observations one step at a time (not sequences),
    the FNO layers act as a learned nonlinear transform on the hidden state.
    """
    ...
    def __init__(
        ...
        from kalman_lstm import FourierNeuralOperatorLayer
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        # FNO expects (seq, batch, hidden) -- wrap single step as seq_len=1
        h_seq = h.unsqueeze(0)
        for fno in self.fno_layers:
            h_seq = fno(h_seq)
        return h_seq.squeeze(0)


class NeuralODEKalmanLSTMExtractor(BaseFeaturesExtractor):
    """Neural ODE-enhanced: Kalman-LSTM -> ODE continuous refinement.

    The ODE integrates dh/dt = f_θ(h) from t=0 to t=1 via RK4,
    acting as a continuous-depth nonlinear refinement of the
    Kalman-filtered hidden state. Works on single observations.
    """
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        p = self.cell.init_covariance(batch, device)
        h, c, p = self.cell(observations, h, c, p)
        h = self.ode(h)  # continuous refinement
        return h
...
# ============================================================
# Evaluation -- uses VecNormalize for proper obs scaling
# ============================================================
def evaluate_agent(
    model,
    features: np.ndarray,
    ...
    """Run the agent on test data with proper normalization."""
...
def train(args):
    ...
    if model_label == "baseline":
        extractor_cls = KalmanLSTMExtractor
        extractor_kwargs = dict(hidden_size=args.hidden_size)
    elif model_label == "fno":
        extractor_cls = FNOKalmanLSTMExtractor
        extractor_kwargs = dict(
            hidden_size=args.hidden_size,
            n_fno_layers=args.n_fno_layers,
            n_modes=args.n_modes,
        )
    elif model_label == "ode":
        extractor_cls = NeuralODEKalmanLSTMExtractor
        extractor_kwargs = dict(
            hidden_size=args.hidden_size,
    ...
    print(f"\n  Model: {model_label} | Params: {total_params:,}")
...
# ============================================================
def parse_args():
    parser = argparse.ArgumentParser(description="PPO Trading Agent -- Ablation Study")
